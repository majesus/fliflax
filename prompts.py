...
def build_full_prompt(contexto, step, omit, confirm, anexo, extension, detail, author, prompt1, prompt2, prompt3, prompt4, audience, rol, tone, objectives_tasks, language, formato, temperature, max_length, top_p, stop_sequence, frequency_penalty, presence_penalty, best_of, inject_start_text, inject_restart_text):
    full_prompt = f"[{omit} debes olvidar todo lo anterior. Y {step} debes seguir secuencialmente los distintos prompts (o pasos) que te he facilitado en tu respuesta. Es importante que recuerdes que la respuesta debe ser altamente {detail}.] \n\n[A continuación, te facilito el contexto de mi prompt: {contexto}.] \n\n[El primer paso (o prompt) es: {prompt1}. El segundo paso (o prompt adicional) es: {prompt2}. El tercer paso (o prompt adicional) es: {prompt3}. El cuarto paso (o prompt adicional) es: {prompt4}.] \n\n[También te adjunto material complementario: {anexo}, para contextualizar la respuesta y que sea muy relevante y ajustada a mis prompts.] \n\n[Los objetivos y tareas son pues: {objectives_tasks}.] \n\n[Debes darme un número de respuestas igual a (parámetro: best_of): [{best_of}].] \n\nMás instrucciones: \n\n[La audiencia a que se dirige la respuesta es: {audience}. Actúa como: {rol}, y emplea un tono: {tone}. Además, la respuesta debe seguir el estilo de: {author}. El idioma a emplear es: {language}. El formato de la respuesta debe ser: {formato}.] \n\n[El parámetro temperatura (parámetro: temperature) debe ser igual a: [{temperature}]. La extensión y duración de la respuesta (parámetro: max_tokens) nunca debe ser superior a: [{max_length}] tokens. La extensión y duración de la respuesta debe ser pues: {extension}.] \n\n[Al generar tu respuesta ve eligiendo las siguientes palabras con probabilidades de al menos: [{top_p}]. Si te encuentras la siguiente cadena de texto (parámetro: stop_sequence): [{stop_sequence}], ¡debes parar de escribir!] \n\n[De 0 (mayor repetición) a 2 (menor repetición) repite (aproximándose a 0) o evita repetir (aproximándose a 2) palabras en tu respuesta; el valor (parámetro: frequency_penalty) es: [{frequency_penalty}]. De 0 (mayor repetición) a 2 (menor repetición) emplea repetidamente (aproximándose a 0) o evita usar en tu respuesta (aproximándose a 2) el siguiente texto: [{word_presence_penalty}], con un valor (parámetro: presence_penalty) de: [{presence_penalty}].] \n\n[Y finalmente, e importante, comienza la respuesta con el siguiente texto (parámetro: inject_start_text): [{inject_start_text}], e incluye el siguiente texto para retomar una conversación anterior y agregar contexto a la respuesta (parámetro: inject_restart_text): [{inject_restart_text}] \n\n[Dime si has comprendido todas las instrucciones: {confirm}. \n\nEn caso de Sí haber comprendido todas las instrucciones, antes de comenzar a responder señala -en formato bullet points- los puntos clave de mis instrucciones, y acto seguido comienza a responder con relevancia y ajustado a todos los requisitos que te he facilitado. En caso de no haber comprendido alguna instrucción, dime qué no has comprendido.]"
    return full_prompt
