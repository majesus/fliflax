...
def build_full_prompt(contexto, step, omit, confirm, anexo, extension, detail, author, prompt1, prompt2, prompt3, prompt4, audience, rol, tone, objectives_tasks, language, formato, temperature, max_length, top_p, stop_sequence, frequency_penalty, presence_penalty, best_of, inject_start_text, inject_restart_text):
    full_prompt = f"[{omit} olvida (u olvides) todo lo anterior. Y {step} sigue (o sigas) los distintos prompts (o pasos) que te he facilitado en tu respuesta . Importante: la respuesta debe ser altamente {detail}. Te facilito el contexto de mi prompt: {contexto}. EL primer paso (o prompt) es: {prompt1}. El segundo paso (o prompt adicional) es: {prompt2}. El tercer paso (o prompt) es: {prompt3}. El cuarto paso (o prompt) es: {prompt4}] [La audiencia a que se dirige la respuesta es: {audience}. Actúa como: {rol}, y emplea un tono: {tone}. Además, la respuesta debe seguir el estilo de: {author}. Los objetivos y tareas que te asigno en tu respuesta, son: {objectives_tasks}. El idioma a emplear es: {language}. El formato de la respuesta debe ser: {formato}. También te adjunto material complementario: {anexo}, para que la respuesta sea muy relevante y ajustada a mis prompts.] [El parámetro temperatura (parámetro: temperature) debe ser igual a: [{temperature}]. La extensión y duración de la respuesta (parámetro: maximum_length) NUNCA debe ser superior a: [{max_length}] tokens. La extensión y duración de la respuesta debe ser pues: {extension}. Al generar tu respuesta ve eligiendo las siguientes palabras con probabilidades de al menos: [{top_p}]. Si te encuentras la siguiente cadena de texto: [{stop_sequence}], ¡debes parar de escribir! De 0 (mayor repetición) a 2 (menor repetición) repite (aproximándose a 0) o evita repetir (aproximándose a 2) palabras en tu respuesta; el valor es: [{frequency_penalty}]. De 0 (mayor repetición) a 2 (menor repetición) emplea repetidamente (aproximándose a 0) o evita usar en tu respuesta (aproximándose a 2) las siguientes palabras: [{word_presence_penalty}], con un valor de: [{presence_penalty}]. Debes darme un número de respuestas igual a: [{best_of}]. Y finalmente, e importante, comienza la respuesta con las siguientes palabras: [{inject_start_text}], y finaliza cada respuesta -independientemente de cuántas respuestas te haya pedido- con las siguientes palabras: [{inject_restart_text}] [¿Me has comprendido? {confirm}. En caso de Sí haberme comprendido, antes de tu respuesta señala con formato bullet points los puntos clave de mi prompt, y comienza a responder con relevancia y ajustado a todos los requisitos que te he facilitado. En caso contrario, dime qué no has comprendido.]"
    return full_prompt
