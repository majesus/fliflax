...
def build_full_prompt(prompt, audience, tone, objectives_tasks, language, temperature, max_length, top_p, stop_sequence, frequency_penalty, presence_penalty, best_of, inject_start_text, inject_restart_text):
    full_prompt = f"{prompt} [Audiencia: {audience}, Tono: {tone}, Objetivos y tareas: {objectives_tasks}, Idioma: {language}, temperature: {temperature}, maximum_length: {max_length}, elige siempre la siguiente palabra en la respuesta con una probabilidad de al menos: {top_p}, si te encuentras con la siguiente cadena de texto ¡para!: {stop_sequence}, de 0 (mayor repetición) a 2 (menor repetición) repite las palabras en tu respuesta: {frequency_penalty}, [de 0 (mayor repetición) a 2 (menor repetición) evita las siguientes palabras: {word_presence_penalty}, con un valor de: {presence_penalty}], dame un número de respuestas igual a: {best_of}, comienza la respuesta con las siguientes palabras: {inject_start_text}, y finaliza siempre la respuesta con las siguientes palabras: {inject_restart_text}]"
    return full_prompt
