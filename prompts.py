...
def build_full_prompt(prompt, audience, tone, objectives_tasks, language, temperature, max_length, top_p, stop_sequence, frequency_penalty, presence_penalty, best_of, inject_start_text, inject_restart_text):
    full_prompt = f"{prompt} [Audiencia: {audience}, Tono: {tone}, Objetivos y tareas: {objectives_tasks}, Idioma: {language}. EL parámetro temperature debe ser igual a: {temperature}. La extensión y duración de la respuesta no debe ser superior a: {max_length} palabras. Al generar tu respuesta elige siempre la siguiente palabra con probabilidad de al menos: {top_p}. Si te encuentras la siguiente cadena de texto ¡para d escribir!: {stop_sequence}. De 0 (mayor repetición) a 2 (menor repetición) repite (aproximándose a 0) o evita repetir (aproximándose a 2) palabras en tu respuesta: {frequency_penalty}. De 0 (mayor repetición) a 2 (menor repetición) repite (aproximándose a 0) o evita (aproximándose a 2) las siguientes palabras: {word_presence_penalty}, con un valor de: {presence_penalty}]. Ofréceme un número de respuestas igual a: {best_of}. Y finalmente, comienza la respuesta con las siguientes palabras: {inject_start_text}, y finaliza la repsuesta siempre con las siguientes palabras: {inject_restart_text}]"
    return full_prompt
