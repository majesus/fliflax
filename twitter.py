...
with st.sidebar.form("my_form"):
    ...
    desde_fecha = st.date_input('¿Desde qué fecha?',value = dt.datetime.now(), label ="date_min")
    hasta_fecha = st.date_input('¿Hasta qué fecha?',value = dt.datetime.now(), label ="date_max")
    ...
    st.form_submit_button("Descargar")
#-----------------------------------------------------------------#
#-----------------------------------------------------------------#
...
for i,tweet in enumerate(sntwitter.TwitterSearchScraper(username).get_items()):
    if i>count:
        break
    tweets_list2.append([tweet.date, tweet.id, tweet.content, tweet.url, tweet.username])
