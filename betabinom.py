...
try:
    ...
    beta = 0.125
    n = 5
    st.markdown('<p style="font-family:Consolas; color:Red; font-size: 14px;"><b>Se ha producido una excepción al proponerse un valor de A2 que provoca una división por 0. Abajo te mostramos los resultados con los datos que proponemos por defecto al entrar en la aplicación.</b></p>', unsafe_allow_html=True)
#----------------------------------------------------#
x = np.arange(1,n+1)
alphas = alpha
betas = beta
